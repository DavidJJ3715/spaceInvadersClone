    ...
    while not user.isDead:
        for event in p.event.get(): 
            ...
            match user.update(keys): #Send the list of keypresses to the user to determine what to do
                case p.K_SPACE:
                    if start: #If the game hasn't been started yet
                ...
                case p.K_ESCAPE:
                    if not start: #The game is started so go to the pause menu
        ...
        if start: #The game hasn't started yet, so flash the splash screen
            increaseAlpha, alpha = func.drawStartText(increaseAlpha, alpha, fadeSpeed, startFont, screen)
        else: #Core game functionality
